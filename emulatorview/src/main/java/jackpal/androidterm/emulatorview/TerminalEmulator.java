...
class TerminalEmulator {
  ...
  public void updateSize(int columns, int rows) {
    ...
    if (!fastResize) {
      ...
      charAtCursor =
          screen.getSelectedText(cursorColor, mCursorCol, mCursorRow, mCursorCol, mCursorRow);
      ...
      case 'n': // Esc [ Pn n - ECMA-48 Status Report Commands
        ...
        switch (getArg0(0)) {
          ...
          case 6: // Cursor position report (CPR):
            ...
            // the cursor location.
            byte[] cpr =
                String.format(Locale.US, "\033[%d;%dR", mCursorRow + 1, mCursorCol + 1).getBytes();
    ...
    for (int i = 0; i <= mArgIndex; i++) {
      ...
      if (code == 0) { // reset
        ...
        mEffect |= TextStyle.fxBold;
          ...
          == 3) { // italics, but rarely used as such; "standout" (inverse colors) with TERM=screen
  ...
  private void emit(int c, int style) {
    ...
    if (autoWrap) {
      ...
      if (mAboutToAutoWrap) mScreen.setLineWrap(mCursorRow);
    }
   ...
   *
   * @param c A char[2] containing either a single UTF-16 char or a surrogate pair to be sent to the
   * screen.
   */
