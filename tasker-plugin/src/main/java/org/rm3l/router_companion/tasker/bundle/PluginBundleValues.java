...
public final class PluginBundleValues {
   ...
   * Type: {@code int}.
   *
   * <p>versionCode of the plug-in that saved the Bundle.
   */
  /*
  ...
  public static Bundle generateBundle(
    ...
    if (customCommandConfiguration != null) {
      if (isVariableCustomCommand) {
        result.putString(
            BUNDLE_COMMAND_CUSTOM_VARIABLE_NAME, customCommandConfiguration.toString());
      } else {
    ...
    if (supportedCommand != null) {
      ...
      if (!TextUtils.isEmpty(supportedCommand.paramHumanReadableHint)) {
        result.putString(
            BUNDLE_COMMAND_SUPPORTED_PARAM_HINT, supportedCommand.paramHumanReadableHint);
      }
    }
   ...
   *
   * <p>This method will not mutate {@code bundle}.
   *
