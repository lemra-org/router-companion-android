        ...
        protected ImmutablePair<Response<ResponseBody>, ? extends Exception> doWork() {
            ...
            try {
                ...
                if (includeScreenshot) {
                    ...
                    if (preferences.contains(Constants.AWS_S3_FEEDBACK_PENDING_TRANSFER_PREF)) {
                        pendingTransferId = preferences
                                .getInt(Constants.AWS_S3_FEEDBACK_PENDING_TRANSFER_PREF, -1);
                    } else {
                    ...
                    if (pendingTransferId != null && pendingTransferId != -1) {
                        if (transferUtility.cancel(pendingTransferId)) {
                            preferences.edit()
                                    .remove(Constants.AWS_S3_FEEDBACK_PENDING_TRANSFER_PREF)
                                    .apply();
                        }
                    }
                    ...
                    //Save transfer ID
                    preferences.edit()
                            .putInt(Constants.AWS_S3_FEEDBACK_PENDING_TRANSFER_PREF,
                ...
                final Response<ResponseBody> response = mDoorbellService
                        .submitFeedbackForm(
                                Constants.DOORBELL_APPID, Constants.DOORBELL_APIKEY,
                                emailText,
                                String.format("%s\n\n" +
                                                "-------\n" +
                                                "%s" +
