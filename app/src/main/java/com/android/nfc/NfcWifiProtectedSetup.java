      ...
      if (new String(record.getType()).equals(NFC_TOKEN_MIME_TYPE)) {
        ...
        if (ssidStringIndex > 0) {
          ...
          if (networkKeyStringIndex > 0) {
            ...
            try {
              ssidSize =
                  hexStringPayload.substring(ssidStringIndex, ssidStringIndex + SIZE_FIELD_WIDTH);
            ...
            try {
              networkKeySize =
                  hexStringPayload.substring(
                      networkKeyStringIndex, networkKeyStringIndex + SIZE_FIELD_WIDTH);
            } catch (IndexOutOfBoundsException ex) {
              return null;
            }
