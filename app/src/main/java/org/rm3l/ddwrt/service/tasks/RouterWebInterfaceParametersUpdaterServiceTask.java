...
import static org.rm3l.ddwrt.resources.conn.NVRAMInfo.HTTP_ENABLE;
...
import static org.rm3l.ddwrt.utils.DDWRTCompanionConstants.DEFAULT_HTTP_PORT;
...
public class RouterWebInterfaceParametersUpdaterServiceTask extends AbstractBackgroundServiceTask {
    ...
    public void runBackgroundServiceTask(@NonNull Router router) throws Exception {
        ...
        final String httpEnabledFromPrefs =
                d(routerPreferences.getString(HTTP_ENABLE, null));
        if (!Objects.equal(httpEnabled, httpEnabledFromPrefs)) {
            editor.putString(HTTP_ENABLE, e(httpEnabled));
            changed = true;
        }

        final String httpsEnabledFromPrefs =
                d(routerPreferences.getString(HTTPS_ENABLE, null));
        if (!Objects.equal(httpsEnabled, httpsEnabledFromPrefs)) {
            editor.putString(HTTPS_ENABLE, e(httpsEnabled));
            changed = true;
        }

        final String remoteMgmtHttpsFromPrefs =
                d(routerPreferences.getString(REMOTE_MGT_HTTPS, null));
