...
//                finish();
    ...
    static void saveRouterUuidPref(Context context, int appWidgetId, String text) {
        ...
        if (routerUuid != null) {
            final SharedPreferences routerPrefs = context.getSharedPreferences(routerUuid, Context.MODE_PRIVATE);
            if (routerPrefs != null) {
                final HashSet<String> widgetsWol = new HashSet<>(routerPrefs.getStringSet(WIDGETS_WOL, new HashSet<String>()));
                widgetsWol.add(String.valueOf(appWidgetId));
                routerPrefs.edit()
                        .putStringSet(WIDGETS_WOL, widgetsWol)
                        .apply();
            }
        }
            ...
            if (routerPrefs != null) {
                ...
                if (widgetsWol != null) {
                    ...
                    for (final String widgetWol : widgetsWol) {
                        if (widgetWol == null ||
                                widgetWol.equalsIgnoreCase(appWidgetIdStr)) {
                            continue;
                        }
                        widgetsWolNew.add(appWidgetIdStr);
                    }
                    routerPrefs.edit()
                            .putStringSet(WIDGETS_WOL, widgetsWolNew)
                            .apply();
                }
            }
        }
...
//                }
        ...
        int i = 1;
        mCurrentItemPos = -1;
        final String selectedRouterUuid = loadRouterUuidPref(WOLWidgetConfigureActivity.this, mAppWidgetId);
