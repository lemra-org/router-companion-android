...
public class ProcNetDevNetworkData {
  ...
  public ProcNetDevNetworkData(String interfaceName, long rxBytes, int rxPackets, int rxErrors,
      int rxDropped, int rxFifo, int rxFrame, int rxCompressed, int rxMulticast, long txBytes,
      int txPackets, int txErrors, int txDropped, int txFifo, int txCollisions, int txCarrierErrors,
      int txCompressed) {

    this.interfaceName = interfaceName;
    this.mReceive = new ProcNetDevReceive(rxBytes, rxPackets, rxErrors, rxDropped, rxFifo, rxFrame,
        rxCompressed, rxMulticast);
    this.mTransmit =
        new ProcNetDevTransmit(txBytes, txPackets, txErrors, txDropped, txFifo, txCollisions,
  ...
  @Override public String toString() {
    return interfaceName
        + ", "
        + String.valueOf(this.mReceive.getRxBytes() + ", ")
