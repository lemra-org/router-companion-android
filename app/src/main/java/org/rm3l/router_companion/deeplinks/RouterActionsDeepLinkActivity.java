...
public class RouterActionsDeepLinkActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!getSharedPreferences(DEFAULT_SHARED_PREFERENCES_KEY, Context.MODE_PRIVATE)
            ...
            Toast.makeText(this, "DD-WRT Companion: Action denied because user disabled 3rd-party integration.", Toast.LENGTH_LONG).show();
            finish();
            return;
        }
...
//                    startActivityForResult(i, PIN_LOCK_REQUEST_CODE);
            ...
            if (!handleParametersUponPasswordCheck()) return;
        }
    ...
    private boolean handleParametersUponPasswordCheck() {
        ...
        final SharedPreferences globalPrefs = getSharedPreferences(DEFAULT_SHARED_PREFERENCES_KEY,
                Context.MODE_PRIVATE);

        final String action = Strings.nullToEmpty(mParameters.getString("action"))
                .toLowerCase();
        ...
        for (final Router router : mRouters) {
            ...
            switch (action) {
                ...
                case EXEC_CUSTOM:
                    final String cmd = Strings.nullToEmpty(mParameters.getString("cmd"))
                            .toLowerCase();
                    if (cmd.isEmpty()) {
                ...
                case EXEC_FILE:
                    final String resourceFile = Strings.nullToEmpty(mParameters.getString("file"))
                            .toLowerCase();
                ...
                case WAKE_ON_LAN:
                case WOL: {
                    final String deviceMac = Strings.nullToEmpty(mParameters.getString("mac"))
                            .toLowerCase();
                ...
                case DISABLE_WAN_POLICY:
                    final String policyName = Strings.nullToEmpty(mParameters.getString("policy"))
                            .toLowerCase();
