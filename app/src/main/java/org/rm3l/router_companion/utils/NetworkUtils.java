...
public final class NetworkUtils {
  ...
  private static class FirebaseDynamicLinksServiceHolder {
    private static final FirebaseDynamicLinksService FIREBASE_DYNAMIC_LINKS_SERVICE =
        NetworkUtils.createApiService(
            null, FIREBASE_DYNAMIC_LINKS_BASE_URL, FirebaseDynamicLinksService.class);
  }
  ...
  private static OkHttpClient getHttpClientInstance(
      @Nullable final Context context, @Nullable final Interceptor... interceptors) {
    final OkHttpClient.Builder builder =
        new OkHttpClient.Builder().addInterceptor(new UserAgentInterceptor());
  ...
  public static void getWifiName(
      @Nullable Activity currentActivity, @NonNull OperationCallback<String, Void> callback) {
    ...
    PermissionsUtils.requestPermissions(
        currentActivity,
        Collections.singletonList(permission.ACCESS_COARSE_LOCATION),
        () -> {
  ...
  public static class AuthenticationInterceptor implements Interceptor {
    ...
    public okhttp3.Response intercept(@NonNull Chain chain) throws IOException {
      ...
      final Request.Builder builder = original.newBuilder().header("Authorization", authToken);
