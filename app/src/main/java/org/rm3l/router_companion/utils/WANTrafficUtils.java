...
public final class WANTrafficUtils {
  ...
  @NonNull
  public static NVRAMInfo computeWANTrafficUsageBetweenDates(
      @NonNull final DDWRTCompanionDAO dao,
  ...
  public static String getSqliteFormattedDate(final String ddwrtRawMonthYear, final int dayNum) {
    ...
    try {
      date =
          DDWRT_MONTHLY_TRAFFIC_DATE_READER.parse(
  ...
  @NonNull
  public static NVRAMInfo getTrafficDataNvramInfoAndPersistIfNeeded(
      Context ctx, Router router, SharedPreferences globalPreferences, DDWRTCompanionDAO dao)
      throws Exception {
  ...
  public static List<WANTrafficData> getWANTrafficDataByRouterBetweenDates(
    ...
    FirebaseCrashlytics.getInstance()
        .log("<cycleStart,cycleEnd>=<" + cycleStart + "," + cycleStart + ">");
    return dao.getWANTrafficDataByRouterBetweenDates(router, cycleStart, cycleEnd);
  }

  public static void retrieveAndPersistMonthlyTrafficData(
    ...
    for (final Map.Entry<Object, Object> dataEntrySet : data.entrySet()) {
      ...
      for (final String dailyInOutTraffData : dailyTraffDataList) {
        ...
        if (inTraff == null
            || inTraff.trim().isEmpty()
            || outTraff == null
            || outTraff.trim().isEmpty()) {
          continue;
        }
        ...
        // there is an "ON CONFLICT REPLACE" constraint that will make the DB update the record if
        // needed
