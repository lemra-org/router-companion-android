...
public final class NetworkTopologyMapTileWorker {
  ...
  public static NVRAMInfo getDataForNetworkTopologyMapTile(
    ...
    try {
      nvramInfoTmp =
          SSHUtils.getNVRamInfoFromRouter(
              ...
              // Apparently, not by Stock Tomato, but other mods might support it (e.g, Tomato by
              // Shibby)
      ...
      final String[] activeClients =
          SSHUtils.getManualProperty(
              context, router, globalSharedPreferences, "arp -a 2>/dev/null");
      if (activeClients != null) {
        nvramInfo.setProperty(
            NVRAMInfo.Companion.getNB_ACTIVE_CLIENTS(), Integer.toString(activeClients.length));
      }
      ...
      if (activeDhcpLeases != null) {
        if (activeDhcpLeases.length == 0 || !"N_A".equals(activeDhcpLeases[0])) {
          nvramInfo.setProperty(
              NVRAMInfo.Companion.getNB_DHCP_LEASES(), Integer.toString(activeDhcpLeases.length));
        } else {
          // File does not exist
      ...
      try {
        ...
        if (wanPublicIpCmdStatus == null || wanPublicIpCmdStatus.length == 0) {
          ...
          if (Patterns.IP_ADDRESS.matcher(wanPublicIp).matches()) {
            nvramInfo.setProperty(
                NVRAMInfo.Companion.getINTERNET_CONNECTIVITY_PUBLIC_IP(), wanPublicIp);
