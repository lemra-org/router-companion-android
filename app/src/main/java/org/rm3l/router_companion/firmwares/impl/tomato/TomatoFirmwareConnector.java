...
public class TomatoFirmwareConnector extends AbstractRouterFirmwareConnector {
  ...
  public static final String TOMATO_SCM_CHANGESET_URL_BASE =
      TOMATO_SCM_URL + "/tag/refs/tags/tomato-";
  ...
  private static NVRAMInfo parseDataForStorageUsageTile(
  ...
  @Override public String getScmChangesetUrl(String changeset) {
    ...
    final List<String> stringList =
        Splitter.on(".").omitEmptyStrings().trimResults().splitToList(changeset);
    if (stringList.size() < 2) {
      return null;
    }
    return String.format("%s%s%s", TOMATO_CHANGELOG_BASE_URL, stringList.get(0), stringList.get(1));
  }
}
