...
public class ReviewStep extends MaterialWizardStep {
  ...
  public class CheckRouterConnectionAsyncTask
      extends AsyncTask<
          Void, Void, CheckRouterConnectionAsyncTask.CheckRouterConnectionAsyncTaskResult<Router>> {
    ...
    @Nullable
    @Override
    protected CheckRouterConnectionAsyncTask.CheckRouterConnectionAsyncTaskResult<Router>
      ...
      return new CheckRouterConnectionAsyncTask.CheckRouterConnectionAsyncTaskResult<>(
          result, exception);
    }
      ...
      if (isAdded() && checkActualConnection) {
        checkingConnectionDialog =
            Utils.buildAlertDialog(
                ...
                null,
                String.format(
                    "Hold on - checking connection to router '%s'...", router.getRemoteIpAddress()),
                false,
                false);
  ...
  private class DDWRTCompanionExceptionForConnectionChecksException
    ...
    private DDWRTCompanionExceptionForConnectionChecksException(
  ...
  public View onCreateView(
    ...
    if (tag != null) {
      try {
        final RouterWizardAction routerWizardAction =
    ...
    FirebaseCrashlytics.getInstance()
        .log("<routerSelected=" + routerSelected + ",action=" + action + ">");
    ...
    routerCustomIconContainer =
        v.findViewById(R.id.wizard_add_router_review_router_custom_icon_container);
    routerCustomIconPreview =
        v.findViewById(R.id.wizard_add_router_review_router_icon_custom_preview);
    ...
    authMethodHidden.addTextChangedListener(
        new TextWatcher() {
          @Override
    ...
    routerCustomIconPathView.addTextChangedListener(
        new TextWatcher() {
          @Override
          public void afterTextChanged(Editable s) {
            final String valueString = s.toString();
  ...
  private Router buildRouter() {
    ...
    if (Utils.isDemoRouter(router)) {
      ...
      if (fwPositionInStringArrayValues < 0) {
        ...
        ReportingUtils.reportException(
            getContext(), new DDWRTCompanionException("fwPositionInStringArrayValues < 0"));
        ...
        if (fwPositionInStringArrayValues < fwStringArray.length) {
          ...
          FirebaseCrashlytics.getInstance()
              .log("fwSelection=" + fwSelection + ", for '" + routerFirmware + "'");
    ...
    //        router.setStrictHostKeyChecking(((CheckBox)
    // d.findViewById(R.id.router_add_is_strict_host_key_checking)).isChecked());

    //        final String password = ((EditText)
    // d.findViewById(R.id.router_add_password)).getText().toString();
    //        final String privkey = ((TextView)
    // d.findViewById(R.id.router_add_privkey_path)).getText().toString();
    ...
    if (!isNullOrEmpty(privkeyContent)) {
      ...
      // JcePEMDecryptorProviderBuilder().build(nullToEmpty(password).toCharArray());
      //            JcaPEMKeyConverter converter = new JcaPEMKeyConverter().setProvider("SC");
      //            KeyPair kp;
      //            if (object instanceof PEMEncryptedKeyPair) {
      //                FirebaseCrashlytics.getInstance().log( "Encrypted key - we will use provided
      // password");
      //                kp = converter.getKeyPair(((PEMEncryptedKeyPair)
      // object).decryptKeyPair(decProv));
      //            } else {
      //                FirebaseCrashlytics.getInstance().log( "Unencrypted key - no password
      // needed");
