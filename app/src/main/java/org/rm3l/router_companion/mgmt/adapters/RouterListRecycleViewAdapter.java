...
public class RouterListRecycleViewAdapter
    ...
    mFilter = new Filter() {
      @Override protected FilterResults performFiltering(final CharSequence constraint) {
        ...
        if (TextUtils.isEmpty(constraint)) {
          ...
          //Filter routers list
          oReturn.values = FluentIterable.from(routers).filter(new Predicate<Router>() {
            @Override public boolean apply(Router input) {
              if (input == null) {
                return false;
              }
  ...
  public void setRoutersList(final List<Router> results) {
    ...
    if (routersList != null) {
      ...
      Collections.sort(routersList, new Comparator<Router>() {
        @Override public int compare(Router o1, Router o2) {
  ...
  @Override public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
    ...
    holder.routerIp.setText(isDemoRouter ? RouterCompanionAppConstants.DEMO
    ...
    if (Strings.isNullOrEmpty(routerModelStr) || Utils.isDemoRouter(routerAt)) {
      ...
      final String[] opts = new String[] {
          "w_300", "h_300", "q_100", "c_thumb", "g_center", "r_20", "e_improve",
          "e_make_transparent", "e_trim"
      };
    ...
    holder.itemView.post(new Runnable() {
      ...
      @Override public void run() {
        ...
        setClickListenerForNestedView(holder.routerViewParent, new View.OnClickListener() {
              @Override public void onClick(View v) {
                //Open Router
                doOpenRouterDetails(routerAt);
              }
  ...
  @Nullable public Integer findRouterPosition(@Nullable final String routerUuid) {
          ...
          new Thread(new Runnable() {
            @Override public void run() {
              try {
                ...
                SSHUtils.runCommands(activity,
                    activity.getSharedPreferences(DEFAULT_SHARED_PREFERENCES_KEY,
                        Context.MODE_PRIVATE), router, Joiner.on(" ; ").skipNulls(),
                    ...
                    "iptables -D FORWARD -j DDWRTCompanion", "iptables -F DDWRTCompanion",
                    "iptables -X DDWRTCompanion",

                    "rm -f "
                        + RouterCompanionAppConstants.WRTBWMON_DDWRTCOMPANION_SCRIPT_FILE_PATH_REMOTE,
  ...
  @NonNull public List<Integer> getSelectedItems() {
  ...
  @Override public void onItemDismiss(final RecyclerView mRecyclerView,
  ...
  private void openAddRouterForm() {
    if (!(activity instanceof FragmentActivity)) {
      ReportingUtils.reportException(null,
          new IllegalStateException("activity is NOT an FragmentActivity"));
      return;
    }
  ...
  class RouterItemMenuOnClickListener implements PopupMenu.OnMenuItemClickListener {
    ...
    @SuppressLint("DefaultLocale") @Override public boolean onMenuItemClick(MenuItem menuItem) {
