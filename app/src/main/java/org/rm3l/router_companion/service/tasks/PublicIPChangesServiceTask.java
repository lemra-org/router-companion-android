...
public class PublicIPChangesServiceTask extends AbstractBackgroundServiceTask {
  ...
  public PublicIPChangesServiceTask(@NonNull Context ctx) {
    super(ctx);
    this.routerModelUpdaterServiceTask = new RouterModelUpdaterServiceTask(ctx);
  }

  public static void buildNotificationIfNeeded(Context mCtx, @NonNull Router router,
      SharedPreferences routerPreferences, String[] wanPublicIpCmdStatus, String wanIp,
    ...
    if (!mCtx.getSharedPreferences(RouterCompanionAppConstants.DEFAULT_SHARED_PREFERENCES_KEY,
        ...
        .contains(PublicIPChangesServiceTask.class.getSimpleName())) {
      Crashlytics.log(Log.DEBUG, LOG_TAG, "PublicIPChangesServiceTask notifications disabled");
      return;
    }

    final String wanIpFromPrefs = d(routerPreferences.getString(LAST_WAN_IP, null));

    final String wanPublicIpFromPrefs = d(routerPreferences.getString(LAST_PUBLIC_IP, null));
    ...
    if (wanPublicIpCmdStatus == null || wanPublicIpCmdStatus.length == 0) {
      ...
      wanPublicIp = wanPublicIpCmdStatus[wanPublicIpCmdStatus.length - 1].trim();
    ...
    if (!(Objects.equal(wanPublicIp, wanPublicIpFromPrefs) && Objects.equal(wanIp,
      ...
      try {
        ...
        final boolean notificationsEnabled =
            routerPreferences.getBoolean(RouterCompanionAppConstants.NOTIFICATIONS_ENABLE, true);
        ...
        if (!notificationsEnabled) {
          ...
          if (!(wanPublicIpNullOrEmpty && wanIpNullOrEmpty)) {
            ...
            final Bitmap largeIcon = BitmapFactory.decodeResource(mCtx.getResources(),
            ...
            final NotificationCompat.Builder mBuilder =
                new NotificationCompat.Builder(mCtx).setSmallIcon(R.drawable.ic_stat_ip)
            ...
            final boolean isFirstTimeForNotification =
                routerPreferences.getBoolean(IS_FIRST_TIME_PREF_PREFIX + notifyID, true);

            final String bigContentTitle =
                (isFirstTimeForNotification ? "New IP Address" : "IP Address change");
            ...
            //Public IP Address
            final String publicIpLine =
                String.format("Public IP   %s", wanPublicIpNullOrEmpty ? "-" : wanPublicIp);
            final Spannable publicIpSpannable = new SpannableString(publicIpLine);
            publicIpSpannable.setSpan(new StyleSpan(android.graphics.Typeface.BOLD), 0,
                "Public IP".length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            inboxStyle.addLine(publicIpSpannable);

            //WAN IP Address
            final String wanIpLine = String.format("WAN IP   %s", wanIpNullOrEmpty ? "-" : wanIp);
            final Spannable wanIpSpannable = new SpannableString(wanIpLine);
            wanIpSpannable.setSpan(new StyleSpan(android.graphics.Typeface.BOLD), 0,
