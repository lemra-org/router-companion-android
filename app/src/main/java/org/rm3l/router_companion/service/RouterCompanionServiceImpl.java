...
public class RouterCompanionServiceImpl extends Service {
  ...
  private final IRouterCompanionService.Stub mBinder = new IRouterCompanionService.Stub() {
    ...
    @Override public List<RouterInfo> getAllRouters() throws RemoteException {
    ...
    @Override public RouterInfo getRouterByUuid(String routerUuid) throws RemoteException {
      final Router router = mDao.getRouter(routerUuid);
      return router != null ? router.toRouterInfo() : null;
    }

    @Override public RouterInfo getRouterById(int routerId) throws RemoteException {
      final Router router = mDao.getRouter(routerId);
      return router != null ? router.toRouterInfo() : null;
    }

    @Override public List<RouterInfo> lookupRoutersByName(String name) throws RemoteException {
    ...
    @Override public List<ActionLog> getActionsByOrigin(String origin) {
      if (isEmpty(origin)) {
        throw new IllegalArgumentException("Origin must not be blank");
      }
    ...
    @Override public List<ActionLog> getActionsByRouterByOrigin(String routerUuid, String origin) {
    ...
    @Override public List<ActionLog> getActionsByRouterByOriginWithSqlConstraints(String routerUuid,
        String origin, String predicate, String groupBy, String having, String orderBy) {
      if (isEmpty(origin) || isEmpty(routerUuid)) {
        throw new IllegalArgumentException("Origin and Router UUID must not be blank");
      }
      final Collection<ActionLog> actionLogCollection =
          mDao.getActionsByRouterByOrigin(routerUuid, origin, predicate, groupBy, having, orderBy);
      return new ArrayList<>(actionLogCollection);
    }
