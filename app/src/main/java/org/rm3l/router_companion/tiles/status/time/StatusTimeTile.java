  ...
  public void onLoadFinished(Loader<NVRAMInfo> loader, NVRAMInfo data) {
    try {
      ...
      if (!(exception instanceof DDWRTTileAutoRefreshNotAllowedException)) {
        ...
        // NTP Client
        final boolean ntpClientEnabled =
            "1".equals(data.getProperty(NVRAMInfo.Companion.getNTP_ENABLE()));
        ((TextView) this.layout.findViewById(R.id.tile_status_time_ntp_client))
            .setText((ntpClientEnabled ? "En" : "Dis") + "abled");
        if (ntpClientEnabled) {
          // NTP Server
          ((TextView) this.layout.findViewById(R.id.tile_status_time_time_server))
              .setText(data.getProperty(NVRAMInfo.Companion.getNTP_SERVER(), "-"));
        } else {
          // NTP Server
        ...
        ((TextView) this.layout.findViewById(R.id.tile_status_time_summertime))
        ...
        try {
          ...
          try {
            ...
            if (isDemoRouter(mRouter)) {
              nvramInfoTmp =
                  new NVRAMInfo()
                      .setProperty(NVRAMInfo.Companion.getNTP_ENABLE(), "1")
              ...
              nvramInfoTmp =
                  SSHUtils.getNVRamInfoFromRouter(
                      ...
                      NVRAMInfo.Companion.getNTP_MODE(),
                      NVRAMInfo.Companion.getNTP_SERVER(),
                      NVRAMInfo.Companion.getTIME_ZONE(),
                      NVRAMInfo.Companion.getDAYLIGHT_TIME());
            }
            ...
            if (isDemoRouter(mRouter)) {
              ...
              currentDate =
                  SSHUtils.getManualProperty(
                      mParentFragmentActivity, mRouter, mGlobalPreferences, "date");
            }
