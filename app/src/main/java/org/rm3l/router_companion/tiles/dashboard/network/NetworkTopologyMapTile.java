...
public class NetworkTopologyMapTile extends DDWRTTile<NVRAMInfo> {
  ...
  public NetworkTopologyMapTile(
    ...
    speedtestFab.setOnClickListener(
        new View.OnClickListener() {
          @Override
          public void onClick(View v) {
            final Intent speedTestIntent =
                new Intent(mParentFragmentActivity, SpeedTestActivity.class);
  ...
  public void onLoadFinished(Loader<NVRAMInfo> loader, NVRAMInfo data) {
    ...
    try {
      ...
      //            final LinearLayout mapContainerView = (LinearLayout)
      // layout.findViewById(R.id.tile_network_map_container);
      ...
      if (!(exception instanceof DDWRTTileAutoRefreshNotAllowedException)) {
        ...
        // Router Name
        final TextView routerNameView = this.layout.findViewById(R.id.tile_network_map_router_name);
        ...
        routerNameView.setTypeface(
            null, isNullOrEmpty(routerNameToSet) ? Typeface.ITALIC : Typeface.NORMAL);
        routerNameView.setText(
            isNullOrEmpty(routerNameToSet) ? "(empty)" : Utils.truncateText(routerNameToSet, 30));
        ...
        final boolean openvpnClEnabled =
            "1".equals(data.getProperty(NVRAMInfo.Companion.getOPENVPNCL_ENABLE()));
        final String vpnClRemoteServerIp =
            data.getProperty(NVRAMInfo.Companion.getOPENVPNCL_REMOTEIP());
        final String vpnClRemoteServerPort =
            data.getProperty(NVRAMInfo.Companion.getOPENVPNCL_REMOTEPORT());
        final View vpnClImageView = layout.findViewById(R.id.tile_network_map_wan_vpn);
        if (openvpnClEnabled) {
          vpnClImageView.setVisibility(View.VISIBLE);
          vpnClImageView.setOnClickListener(
              new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                  Utils.displayMessage(
                      ...
                      isNullOrEmpty(vpnClRemoteServerIp)
                          ...
                          : String.format(
                              "Secure VPN Connection established with %s%s",
                              vpnClRemoteServerIp,
        ...
        layout
            .findViewById(R.id.tile_network_map_router)
            .setOnClickListener(routerStateClickListener);
        ...
        if (publicIp == null || UNKNOWN.equals(publicIp)) {
          ...
          statusToastMsg =
              ("Your router seems not to be able to reach the Internet. "
          ...
          publicIpViewTextSpannable.setSpan(
              new StyleSpan(android.graphics.Typeface.BOLD),
              "Public IP:\n".length(),
              publicIpViewText.length(),
          ...
          statusUnknownView.setVisibility(View.GONE);
          statusToastMsg =
              ("Your router seems to be able to reach the Internet. "
        ...
        final View.OnClickListener statusViewOnClickListener =
            new View.OnClickListener() {
              @Override
              public void onClick(View v) {
    ...
    return new AsyncTaskLoader<NVRAMInfo>(this.mParentFragmentActivity) {
      ...
      public NVRAMInfo loadInBackground() {
        ...
        try {
          ...
          // We are cloning the Router, with a new UUID, so as to have a different key into the SSH
          // Sessions Cache
          // This is because we are fetching in a quite real-time manner, and we don't want to block
          // other async tasks.
