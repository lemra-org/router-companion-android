...
public class AddWOLHostDialogFragment extends DialogFragment {
  ...
  @Nullable protected SharedPreferences mRouterPreferences;

  protected AlertDialog mWaitingDialog = null;

  @Nullable private ArrayList<String> bcastAddresses;

  private Device mDevice;

  @Nullable private Router mRouter;

  public static AddWOLHostDialogFragment newInstance(
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    mRouterPreferences =
        (routerSelected != null
            ? activity.getSharedPreferences(routerSelected, Context.MODE_PRIVATE)
            : null);
      ...
      if (mRouterPreferences != null) {
        ...
        final Set<String> wolHostMacAddresses =
            mRouterPreferences.getStringSet(
                ADD_OR_EDIT_WOL_HOSTS_MAC_ADDRESSES, new HashSet<String>());
                ...
                if (validForm) {
                  ...
                  if (mRouterPreferences != null) {
                    final Set<String> existingHostNames =
                        new HashSet<>(
                            mRouterPreferences.getStringSet(
                                ADD_OR_EDIT_WOLHOSTS_HOSTNAMES, new HashSet<String>()));
                    existingHostNames.add(mDevice.getAlias());

                    final Set<String> existingHostMacs =
                        new HashSet<>(
                            mRouterPreferences.getStringSet(
                                ADD_OR_EDIT_WOL_HOSTS_MAC_ADDRESSES, new HashSet<String>()));
                    existingHostMacs.add(mDevice.getMacAddress());

                    final Set<String> existingHostPorts =
                        new HashSet<>(
                            mRouterPreferences.getStringSet(
                                ADD_OR_EDIT_WOL_HOSTS_PORTS, new HashSet<String>()));
                  ...
                  if (mWaitingDialog == null) {
                    mWaitingDialog =
                        Utils.buildAlertDialog(
                            getActivity(), null, "Sending magic packet...", false, false);
                  }
                  mWaitingDialog.show();

                  ((TextView) mWaitingDialog.findViewById(android.R.id.message))
                      .setGravity(Gravity.CENTER_HORIZONTAL);
                  ActionManager.runTasks(
                      new WakeOnLANRouterAction(
                          mRouter,
                          getActivity(),
                          getRouterActionListener(),
                          mGlobalPreferences,
                          mDevice,
                          mDevice.getWolPort(),
    ...
    return new RouterActionListener() {
      ...
      public void onRouterActionSuccess(
        ...
        try {
          ...
          if (d == null) {
            displayMessage(
                "WOL Internal Error. Action succeeded, but failed to save entry", Style.INFO);
            Utils.reportException(
                null, new IllegalStateException("WOL Internal Error: Dialog is NULL."));
            return;
          }
  ...
  private void displayMessage(final String msg, final Style style) {
    ...
    Utils.displayMessage(
        ...
        style,
        (ViewGroup)
            (d == null ? getView() : d.findViewById(R.id.wol_host_add_notification_viewgroup)));
  }
