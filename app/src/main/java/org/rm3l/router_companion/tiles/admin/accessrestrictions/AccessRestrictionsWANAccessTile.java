...
public class AccessRestrictionsWANAccessTile
  ...
  public AccessRestrictionsWANAccessTile(@NonNull Fragment parentFragment,
    ...
    super(parentFragment, arguments, router, R.layout.tile_admin_access_restrictions_wan_access,
        null);
  ...
  @Nullable @Override
  protected Loader<AccessRestrictionsWANAccessTile.WANAccessPoliciesRouterData> getLoader(int id,
      Bundle args) {
    return new AsyncTaskLoader<WANAccessPoliciesRouterData>(this.mParentFragmentActivity) {
      @Override public WANAccessPoliciesRouterData loadInBackground() {
        try {
          Crashlytics.log(Log.DEBUG, LOG_TAG, "Init background loader for "
              + AccessRestrictionsWANAccessTile.class
              + ": routerInfo="
              + mRouter
              + " / nbRunsLoader="
              + nbRunsLoader);
          ...
          if (Utils.isDemoRouter(mRouter)) {
            for (int i = 1; i <= 10; i++) {
              final WANAccessPolicy wanAccessPolicy =
                  new WANAccessPolicy().setNumber(i).setName("myWanPolicy " + i);
            ...
            //1- Get all rules first
                        ...
                        filter_rule10=
                         */
            NVRAMInfo nvramInfo = SSHUtils.getNVRamInfoFromRouter(mParentFragmentActivity, mRouter,
            ...
            for (final Map.Entry<Object, Object> entry : entries) {
              ...
              final List<String> statusSplitter =
                  Splitter.on("$NAME:").omitEmptyStrings().trimResults().splitToList(valueStr);
              ...
              if (nvramInfo != null && nvramInfo.getProperty("filter_tod" + keyNb) != null) {
                ...
                final List<String> list =
                    Splitter.on(" ").omitEmptyStrings().trimResults().splitToList(filterTod);
                if (list.size() >= 2) {
                  final String start = list.get(0);
                  final String end = list.get(1);
                  if ("0:0".equals(start) && "23:59".equals(end)) {
          ...
          final WANAccessPoliciesRouterData routerData =
              (WANAccessPoliciesRouterData) new WANAccessPoliciesRouterData().setData(
                  wanAccessPolicies);
  ...
  @Override public void onLoadFinished(Loader<WANAccessPoliciesRouterData> loader,
    ...
    try {
      ...
      if (data == null) {
        data = (WANAccessPoliciesRouterData) new WANAccessPoliciesRouterData().setException(
      ...
      layout.findViewById(R.id.tile_admin_access_restrictions_wan_access_loading_view)
          .setVisibility(View.GONE);

      final TextView errorPlaceHolderView =
          (TextView) this.layout.findViewById(R.id.tile_admin_access_restrictions_wan_access_error);
      ...
      if (!(exception instanceof DDWRTTileAutoRefreshNotAllowedException)) {
        ...
        ((WANAccessRulesRecyclerViewAdapter) mAdapter).setWanAccessPolicies(wanAccessPolicies);
  ...
  static class WANAccessRulesRecyclerViewAdapter
    ...
    @NonNull private final AccessRestrictionsWANAccessTile tile;

    @NonNull private final List<WANAccessPolicy> wanAccessPolicies = new ArrayList<>();
    ...
    @Override public long getItemId(int position) {
      ...
      return ViewIDUtils.getStableId(WANAccessPolicy.class, Integer.toString(itemAt.getNumber()));
    }

    @NonNull public List<WANAccessPolicy> getWanAccessPolicies() {
    ...
    public WANAccessRulesRecyclerViewAdapter setWanAccessPolicies(
      ...
      if (wanAccessPolicies != null) {
        ...
        Collections.sort(this.wanAccessPolicies, new Comparator<WANAccessPolicy>() {
          @Override public int compare(WANAccessPolicy lhs, WANAccessPolicy rhs) {
    ...
    @Override public void onBindViewHolder(final ViewHolder holder, int position) {
      ...
      if (position < 0 || position >= wanAccessPolicies.size()) {
        Crashlytics.log(Log.DEBUG, LOG_TAG, "invalid position for WAN Access Policy Adapter");
        return;
      }
      final WANAccessPolicy wanAccessPolicy = wanAccessPolicies.get(position);
      if (wanAccessPolicy == null) {
        //Report error
        Crashlytics.log(Log.DEBUG, LOG_TAG, "wanAccessPolicy @" + position + " is NULL");
        return;
      }
      ...
      if (daysPattern != null) {
        ...
        final List<String> timeOfDayBufferList =
            Splitter.on(" ").omitEmptyStrings().trimResults().splitToList(daysPattern);
      ...
      holder.internetPolicyDuringSelectedTimeOfDay.setText(wanAccessPolicy.getDenyOrFilter());

      holder.policyHours.setText(wanAccessPolicy.getTimeOfDay());

      //Disable switch button listener
      holder.wanPolicyStateText.setEnabled(true);
      final String status = Strings.nullToEmpty(wanAccessPolicy.getStatus()).trim();
      Integer stateBgColor;
      switch (status) {
        case "0":
        case "$STAT:0":
          //Disabled
          stateBgColor = R.color.win8_red;
          holder.wanPolicyStateText.setText("disabled");
          break;
        case "1":
        case "$STAT:1":
        case "2":
          ...
          new AlertDialog.Builder(tile.mParentFragmentActivity).setIcon(
              ...
              .setPositiveButton("Proceed!", new DialogInterface.OnClickListener() {
                @Override public void onClick(DialogInterface dialog, int which) {
                  holder.wanPolicyStateText.setEnabled(false);
                  ...
                  final NVRAMInfo nvramVarsToSet =
                      new NVRAMInfo().setProperty("filter_dport_grp" + wanAccessPolicyNumber,
                          EMPTY_STRING)
                  ...
                  ActionManager.runTasks(
                      new SetNVRAMVariablesAction(tile.mRouter, tile.mParentFragmentActivity,
                          nvramVarsToSet, false, new RouterActionListener() {
                        @Override
                            ...
                            @Override public void run() {
                              try {
                                Utils.displayMessage(tile.mParentFragmentActivity, String.format(
                                    ...
                                    tile.mRouter.getCanonicalHumanReadableName()), Style.CONFIRM);
                              } finally {
                                holder.wanPolicyStateText.setEnabled(true);
                        ...
                        public void onRouterActionFailure(@NonNull RouterAction routerAction,
                            @NonNull Router router, @Nullable final Exception exception) {
                          tile.mParentFragmentActivity.runOnUiThread(new Runnable() {
                            @Override public void run() {
                              try {
                                Utils.displayMessage(tile.mParentFragmentActivity, String.format(
    ...
    private void toggleWANAccessRestriction(final ViewHolder holder,
        final WANAccessPolicy wanAccessPolicy, final boolean newStatus) {
      holder.wanPolicyStateText.setEnabled(false);
      SnackbarUtils.buildSnackbar(tile.mParentFragmentActivity,
          tile.mParentFragmentActivity.findViewById(android.R.id.content),
          String.format("Going to %sable WAN Access Policy: '%s'", newStatus ? "en" : "dis",
              wanAccessPolicy.getName()), "Undo", Snackbar.LENGTH_LONG, new SnackbarCallback() {
                            ...
                            @Override public void run() {
                              ...
                              try {
                                Utils.displayMessage(tile.mParentFragmentActivity, String.format(
                                    ...
                                    wanAccessPolicy.getName(), newStatus ? "enabled" : "disabled",
                                    tile.mRouter.getCanonicalHumanReadableName()), Style.CONFIRM);
                              } finally {
                            ...
                            @Override public void run() {
                              try {
                                Utils.displayMessage(tile.mParentFragmentActivity, String.format(
                                    ...
                                    newStatus ? "enable" : "disable", wanAccessPolicy.getName(),
                                    tile.mRouter.getCanonicalHumanReadableName(),
                                    Utils.handleException(exception).first), Style.ALERT);
                              } finally {
                                //Revert
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
      ...
      final TextView wanPolicyStateText;
      ...
      public ViewHolder(AccessRestrictionsWANAccessTile tile, View itemView) {
        super(itemView);
        this.tile = tile;
        this.cardView = (CardView) itemView.findViewById(R.id.access_restriction_policy_cardview);

        this.avatarImageView = (ImageView) itemView.findViewById(R.id.avatar);
        this.policyNb =
            (TextView) itemView.findViewById(R.id.access_restriction_policy_cardview_number);
        this.policyName =
            (TextView) itemView.findViewById(R.id.access_restriction_policy_cardview_name);
        this.policyHours =
            (TextView) itemView.findViewById(R.id.access_restriction_policy_cardview_hours);
        this.internetPolicyDuringSelectedTimeOfDay = (TextView) itemView.findViewById(
            R.id.access_restriction_policy_cardview_internet_policy);

        this.daysTextViews[0] =
            (TextView) itemView.findViewById(R.id.access_restriction_policy_cardview_days_sunday);
        this.daysTextViews[1] =
            (TextView) itemView.findViewById(R.id.access_restriction_policy_cardview_days_monday);
        this.daysTextViews[2] =
            (TextView) itemView.findViewById(R.id.access_restriction_policy_cardview_days_tuesday);
        this.daysTextViews[3] = (TextView) itemView.findViewById(
            R.id.access_restriction_policy_cardview_days_wednesday);
        this.daysTextViews[4] =
            (TextView) itemView.findViewById(R.id.access_restriction_policy_cardview_days_thursday);
        this.daysTextViews[5] =
            (TextView) itemView.findViewById(R.id.access_restriction_policy_cardview_days_friday);
        this.daysTextViews[6] =
            (TextView) itemView.findViewById(R.id.access_restriction_policy_cardview_days_saturday);

        this.wanPolicyStateText =
            (TextView) itemView.findViewById(R.id.access_restriction_policy_cardview_status);
        this.menuImageButton =
            (ImageButton) itemView.findViewById(R.id.access_restriction_policy_cardview_menu);
        this.removeImageButton =
            (ImageButton) itemView.findViewById(R.id.access_restriction_policy_remove_btn);
      }
    }
  }

  static class WANAccessPoliciesRouterData extends RouterData<List<WANAccessPolicy>> {
