...
public class WriteWifiConfigToNfcDialog extends AlertDialog implements View.OnClickListener {
  ...
  public WriteWifiConfigToNfcDialog(
      Activity context, final String ssid, final String password, final String mWifiEncType) {
    super(context);
    mContext = context;
    mWakeLock =
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    setButton(
        ...
        mContext.getResources().getString(R.string.write_tag),
  ...
  public void onClick(View v) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
      ...
      //            //TODO Pre-KK devices
      //            // Stop here, we definitely need NFC
      //            Toast.makeText(mContext, "This device doesn't support writing NFC tags.",
  ...
  private void handleWriteNfcEvent(Tag tag) {
    ...
    try {
      if (ndef != null) {
        ...
        setViewText(
            mLabelView,
            "NFC tag is not writeable because it does not support NDEF. "
                + "Please use a different one supporting NDEF technology.");
        final String[] techList = tag.getTechList();
        FirebaseCrashlytics.getInstance()
            .log("Tag does not support NDEF. Tech List: " + Arrays.toString(techList));
      }
    } catch (Exception e) {
