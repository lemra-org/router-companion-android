...
public class DHCPStatusTile extends DDWRTTile<NVRAMInfo> {
  ...
  public DHCPStatusTile(@NonNull Fragment parentFragment, @NonNull Bundle arguments,
      @Nullable Router router) {
    super(parentFragment, arguments, router, R.layout.tile_status_lan_dhcp_status, null);
  }
          ...
          try {
            ...
            if (dhcpStart != null && lanAddr != null) {
              ...
              if (lowAddressSplit != null && lowAddressSplit.size() >= 3) {
                dhcpStartIp =
                    String.format("%s.%s.%s.%s", lowAddressSplit.get(0), lowAddressSplit.get(1),
                        lowAddressSplit.get(2), dhcpStart);
  ...
  @Override public void onLoadFinished(@NonNull Loader<NVRAMInfo> loader,
    ...
    try {
      ...
      final TextView errorPlaceHolderView =
          (TextView) this.layout.findViewById(R.id.tile_status_lan_dhcp_status_error);
      ...
      if (!(exception instanceof DDWRTTileAutoRefreshNotAllowedException)) {
        ...
        //Start IP
        ((TextView) this.layout.findViewById(R.id.tile_status_lan_dhcp_status_start_ip)).setText(
            data.getProperty(DHCP_START_IP, "-"));

        //End IP
        ((TextView) this.layout.findViewById(R.id.tile_status_lan_dhcp_status_end_ip)).setText(
            data.getProperty(DHCP_END_IP, "-"));
