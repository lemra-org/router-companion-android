  ...
  public void onLoadFinished(Loader<None> loader, None data) {
    try { // Set tiles
      ...
      FirebaseCrashlytics.getInstance()
          .log(
              String.format(
                  "<mWanPublicIP=%s, mIPWhoisInfoWithGeo=%s>", mWanPublicIP, mIPWhoisInfoWithGeo));
        ...
        if (mIPWhoisInfoWithGeo == null) {
          data =
              (None)
                  new None()
                      .setException(
                          new DDWRTNoDataException(
                              "Failed to retrieved Public IP Address geolocation data!"));
        } else {
          try {
            ...
            data =
                (None)
                    new None()
                        .setException(
                            new DDWRTNoDataException(
                                "Invalid coordinates - please try again later!"));
          }
        }
        exception = data.getException();
      }

      final TextView errorPlaceHolderView = this.layout.findViewById(R.id.tile_public_ip_geo_error);
    ...
    return new AsyncTaskLoader<None>(this.mParentFragmentActivity) {
      ...
      public None loadInBackground() {
        ...
        try {
          ...
          mParentFragmentActivity.runOnUiThread(
              () ->
                  zoomMapButton.setImageDrawable(
          ...
          try {
            ...
            mWanPublicIP =
                mRouterConnector.getWanPublicIpAddress(
                    mParentFragmentActivity,
                    mRouter,
