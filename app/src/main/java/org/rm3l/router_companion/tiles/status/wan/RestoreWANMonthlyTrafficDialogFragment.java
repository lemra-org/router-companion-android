...
public class RestoreWANMonthlyTrafficDialogFragment extends DialogFragment
  ...
  @NonNull @Override public Dialog onCreateDialog(Bundle savedInstanceState) {
    ...
    builder.setTitle(
        String.format("Restore WAN Traffic Data on '%s' (%s)", mRouter.getDisplayName(),
        ...
        .setMessage(String.format("Browse for a raw backup file and"
                ...
                + "with the firmware and model of your router.", mRouter.getDisplayName(),
            mRouter.getRemoteIpAddress()))
        .setView(view)
        // Add action buttons
        ...
        .setNeutralButton("*Backup*", new DialogInterface.OnClickListener() {
          @Override public void onClick(DialogInterface dialogInterface, int i) {
            displayBackupDialog(
                String.format("'%s' (%s)", mRouter.getDisplayName(), mRouter.getRemoteIpAddress()),
    ...
    if (d != null) {
      ...
      d.findViewById(R.id.router_restore_backup_select_button)
          .setOnClickListener(new View.OnClickListener() {
            @TargetApi(Build.VERSION_CODES.KITKAT) @Override public void onClick(View view) {
              //Open up file picker
              ...
              RestoreWANMonthlyTrafficDialogFragment.this.startActivityForResult(intent,
                  READ_REQUEST_CODE);
            }
          });
      ...
      d.getButton(Dialog.BUTTON_POSITIVE).setOnClickListener(new View.OnClickListener() {
        @Override public void onClick(View view) {
          ...
          if (validForm) {
            ...
            ActionManager.runTasks(new RestoreWANMonthlyTrafficFromBackupAction(mRouter, activity,
                new RouterActionListener() {
                  @Override public void onRouterActionSuccess(@NonNull RouterAction routerAction,
                      @NonNull Router router, Object returnData) {
                    try {
                      Utils.displayMessage(activity, String.format(
                          "Action '%s' executed successfully on host '%s'. "
                              + "Data will refresh upon next sync.", routerAction.toString(),
                    ...
                    try {
                      ...
                      activity.runOnUiThread(new Runnable() {
                        @Override public void run() {
                          ...
                          ((TextView) d.findViewById(R.id.router_restore_backup_path)).setText(
                              null);
        ...
        switch (RouterAction.valueOf(routerAction)) {
          case BACKUP_WAN_TRAFF:
            final int fileType = token.getInt(WAN_MONTHLY_TRAFFIC_BACKUP_FILETYPE,
                BackupWANMonthlyTrafficRouterAction.BackupFileType_RAW);
            final AlertDialog alertDialog = Utils.
                buildAlertDialog(mCtx, null, "Backing up WAN Traffic Data - please hold on...",
                    false, false);
            alertDialog.show();
            ((TextView) alertDialog.findViewById(android.R.id.message)).setGravity(
                Gravity.CENTER_HORIZONTAL);
            ActionManager.runTasks(new BackupWANMonthlyTrafficRouterAction(mRouter, fileType, mCtx,
                new RouterActionListener() {
                  ...
                  @Override public void onRouterActionSuccess(@NonNull RouterAction routerAction,
                    ...
                    try {
                      ...
                      Utils.displayMessage(mCtx, String.format(
                          "Action '%s' executed successfully on host '%s'. "
                      ...
                      mCtx.grantUriPermission(mCtx.getPackageName(), uriForFile,
