...
public class WANConfigTile extends DDWRTTile<NVRAMInfo>
  ...
  public WANConfigTile(
    ...
    // Create Options Menu
    final ImageButton tileMenu = layout.findViewById(R.id.tile_status_wan_config_menu);
  ...
  public void onLoadFinished(@NonNull Loader<NVRAMInfo> loader, @Nullable NVRAMInfo data) {
    try {
      ...
      if (!(exception instanceof DDWRTTileAutoRefreshNotAllowedException)) {
        ...
        // WAN MAC
        final TextView wanMacView = this.layout.findViewById(R.id.tile_status_wan_config_wan_mac);
        ...
        final TextView wanIPView = this.layout.findViewById(R.id.tile_status_wan_config_wan_ip);
        ...
        final TextView rDnsTitle = this.layout.findViewById(R.id.tile_status_wan_config_rdns_title);
        final TextView rDnsTextView = this.layout.findViewById(R.id.tile_status_wan_config_rdns);
        ...
        // DNS
        final TextView wanDNSView = this.layout.findViewById(R.id.tile_status_wan_config_dns);
  ...
  public boolean onMenuItemClick(MenuItem menuItem) {
    ...
    if (itemId == R.id.tile_wan_config_dhcp_release || itemId == R.id.tile_wan_config_dhcp_renew) {
      ...
      SnackbarUtils.buildSnackbar(
          mParentFragmentActivity,
          String.format(
              "WAN DHCP Lease will be %s on '%s' (%s). ",
                              ...
                              new Runnable() {
                                ...
                                public void run() {
                                  try {
                                    ...
                                    layout
                                        .findViewById(R.id.tile_status_wan_config_menu)
                                        .setEnabled(true);
                                  }
                                }
                              });
                                ...
                                public void run() {
                                  ...
                                  try {
                                    ...
                                    if (mLoader != null) {
                                      ...
                                      doneWithLoaderInstance(WANConfigTile.this, mLoader, 1l);
                                    }
                                    layout
                                        .findViewById(R.id.tile_status_wan_config_menu)
                                        .setEnabled(true);
            ...
            try {
              ...
              try {
                ...
                if (isDemoRouter(mRouter)) {
                  final String[] possibleWanProtos =
                      new String[] {"ppoe", "3g", "heartbeat", "disabled", "dhcp"};
                  nvramInfoTmp =
                      new NVRAMInfo()
                          .setProperty(
                              NVRAMInfo.Companion.getWAN_PROTO(),
                  ...
                  nvramInfoTmp =
                      SSHUtils.getNVRamInfoFromRouter(
                          ...
                          mGlobalPreferences,
                          NVRAMInfo.Companion.getWAN_PROTO(),
                          NVRAMInfo.Companion.getWAN_3_G_SIGNAL(),
                          NVRAMInfo.Companion.getWAN_HWADDR(),
                          NVRAMInfo.Companion.getWAN_LEASE(),
                          NVRAMInfo.Companion.getWAN_IPADDR(),
                          NVRAMInfo.Companion.getWAN_NETMASK(),
                          NVRAMInfo.Companion.getWAN_GATEWAY(),
                          NVRAMInfo.Companion.getWAN_GET_DNS(),
                          NVRAMInfo.Companion.getWAN_DNS());
                }
                ...
                if (uptimes != null && uptimes.length > 1) {
                   ...
                   */
                  final List<String> sysUptimeSplitList =
                      Splitter.on(" ").trimResults().omitEmptyStrings().splitToList(uptimes[1]);
                  if (sysUptimeSplitList != null && !sysUptimeSplitList.isEmpty()) {
                    try {
                      ...
                      wanConnectionUptimeStr +=
                          String.format(
                              "%d:%02d:%02d", (minutes / 60) % 24, minutes % 60, (int) uptime % 60);

                      nvramInfo.setProperty(
                          NVRAMInfo.Companion.getWAN_CONNECTION_UPTIME(), wanConnectionUptimeStr);
                ...
                if (checkActualInternetConnectivity) {
                  final CharSequence applicationName =
                      Utils.getApplicationName(mParentFragmentActivity);
                  try {
                    ...
                    if (isDemoRouter(mRouter)) {
                      ...
                      if (nbRunsLoaderModulo == 0) {
                        ...
                        nvramInfo.setProperty(INTERNET_CONNECTIVITY_PUBLIC_IP, randomPublicIp);
                        nvramInfo.setProperty(
                            REVERSE_DNS_PTR, p2 + "." + p1 + ".64.52.in-addr.arpa");
                      ...
                      final String wanPublicIpAddress =
                          RouterFirmwareConnectorManager.getConnector(mRouter)
                      ...
                      FirebaseCrashlytics.getInstance()
                          .log("wanPublicIpCmdStatus: " + Arrays.toString(wanPublicIpCmdStatus));
                      if (wanPublicIpCmdStatus.length == 0) {
                        nvramInfo.setProperty(INTERNET_CONNECTIVITY_PUBLIC_IP, NOK);
                      } else {
                        final String wanPublicIp =
                            wanPublicIpCmdStatus[wanPublicIpCmdStatus.length - 1].trim();
                        if (Patterns.IP_ADDRESS.matcher(wanPublicIp).matches()) {
                          nvramInfo.setProperty(INTERNET_CONNECTIVITY_PUBLIC_IP, wanPublicIp);

                          PublicIPChangesServiceTask.Companion.buildNotificationIfNeeded(
                              mParentFragmentActivity,
                              mRouter,
                              wanPublicIpCmdStatus,
                              nvramInfo.getProperty(NVRAMInfo.Companion.getWAN_IPADDR()),
                              null);
                        } else {
                          nvramInfo.setProperty(INTERNET_CONNECTIVITY_PUBLIC_IP, NOK);
                        }
                    ...
                    try {
                      ...
                      FirebaseCrashlytics.getInstance()
                          .log("revDnsCmdStatus: " + Arrays.toString(revDnsCmdStatus));

                      if (revDnsCmdStatus != null && revDnsCmdStatus.length > 0) {
                        nvramInfo.setProperty(
                            REVERSE_DNS_PTR, revDnsCmdStatus[revDnsCmdStatus.length - 1]);
                      }
