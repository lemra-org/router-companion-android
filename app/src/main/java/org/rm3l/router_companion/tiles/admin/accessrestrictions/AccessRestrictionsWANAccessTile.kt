 ...
 */
class AccessRestrictionsWANAccessTile(parentFragment: Fragment,
    arguments: Bundle,
    router: Router?) : DDWRTTile<AccessRestrictionsWANAccessTile.WANAccessPoliciesRouterData>(
    parentFragment, arguments, router, R.layout.tile_admin_access_restrictions_wan_access,
    null), PopupMenu.OnMenuItemClickListener {

  private val addNewButton: FloatingActionButton

  private val mRecyclerView: RecyclerViewEmptySupport = layout.findViewById(
      R.id.tile_admin_access_restrictions_wan_access_ListView) as RecyclerViewEmptySupport
  ...
  init {
    ...
    //
    val emptyView = layout.findViewById(R.id.empty_view) as TextView
    ...
    addNewButton = layout.findViewById(
        R.id.wan_access_restriction_policy_add) as FloatingActionButton
    ...
    //Create Options Menu
    val tileMenu = layout.findViewById(
        R.id.tile_admin_access_restrictions_wan_access_menu) as ImageButton
  ...
  override fun getLoader(id: Int,
      args: Bundle): Loader<AccessRestrictionsWANAccessTile.WANAccessPoliciesRouterData>? {
      ...
      override fun loadInBackground(): WANAccessPoliciesRouterData? {
        try {
          ...
          if (Utils.isDemoRouter(mRouter)) {
            for (i in 1..10) {
              val wanAccessPolicy = WANAccessPolicy().setNumber(i).setName("myWanPolicy " + i)
  ...
  override fun onLoadFinished(loader: Loader<WANAccessPoliciesRouterData>,
      data: WANAccessPoliciesRouterData?) {
    var data = data
    ...
    try {
      ...
      val errorPlaceHolderView = this.layout.findViewById(
          R.id.tile_admin_access_restrictions_wan_access_error) as TextView
  ...
  override fun onMenuItemClick(item: MenuItem): Boolean {
    when (item.itemId) {
      R.id.tile_access_policy_add -> {
        run {
          //TODO
          Toast.makeText(mParentFragmentActivity, "TODO OnMenuItemClick: Add WAN Access Policy",
              Toast.LENGTH_SHORT).show()
        }
        return true
      }
  ...
  internal class WANAccessRulesRecyclerViewAdapter(
      private val tile: AccessRestrictionsWANAccessTile) : RecyclerView.Adapter<WANAccessRulesRecyclerViewAdapter.ViewHolder>() {
      ...
      val removeWanPolicyDialog = AlertDialog.Builder(tile.mParentFragmentActivity).setIcon(
          ...
          .setPositiveButton("Proceed!") { dialog, which ->
            ...
            ActionManager.runTasks(
                SetNVRAMVariablesAction(tile.mRouter, tile.mParentFragmentActivity,
                    ...
                    tile.mParentFragmentActivity.runOnUiThread {
                      try {
                        Utils.displayMessage(tile.mParentFragmentActivity, String.format(
                            ...
                            tile.mRouter!!.canonicalHumanReadableName), Style.CONFIRM)
                      } finally {
                        holder.wanPolicyStateText.isEnabled = true
                    ...
                    tile.mParentFragmentActivity.runOnUiThread {
                      try {
                        Utils.displayMessage(tile.mParentFragmentActivity, String.format(
                            ...
                            wanAccessPolicy.getName(),
                            tile.mRouter!!.canonicalHumanReadableName,
    ...
    private fun toggleWANAccessRestriction(holder: ViewHolder,
      ...
      SnackbarUtils.buildSnackbar(tile.mParentFragmentActivity,
        ...
        override fun onDismissEventTimeout(event: Int, bundle: Bundle?) {
          ...
          val enableStatus = if (!newStatus)
            ...
            ENABLE_2
          ActionManager.runTasks(
              ToggleWANAccessPolicyRouterAction(tile.mRouter, tile.mParentFragmentActivity,
    ...
    internal class ViewHolder(private val tile: AccessRestrictionsWANAccessTile,
        itemView: View) : RecyclerView.ViewHolder(itemView) {

      val cardView: CardView
      val policyNb: TextView
      val policyName: TextView
      val policyHours: TextView
      val internetPolicyDuringSelectedTimeOfDay: TextView
      ...
      init {
        this.cardView = itemView.findViewById(R.id.access_restriction_policy_cardview) as CardView

        this.avatarImageView = itemView.findViewById(R.id.avatar) as ImageView
        this.policyNb = itemView.findViewById(
            R.id.access_restriction_policy_cardview_number) as TextView
        this.policyName = itemView.findViewById(
            R.id.access_restriction_policy_cardview_name) as TextView
        this.policyHours = itemView.findViewById(
            R.id.access_restriction_policy_cardview_hours) as TextView
        this.internetPolicyDuringSelectedTimeOfDay = itemView.findViewById(
            R.id.access_restriction_policy_cardview_internet_policy) as TextView

        this.daysTextViews[0] = itemView.findViewById(
            R.id.access_restriction_policy_cardview_days_sunday) as TextView
        this.daysTextViews[1] = itemView.findViewById(
            R.id.access_restriction_policy_cardview_days_monday) as TextView
        this.daysTextViews[2] = itemView.findViewById(
            R.id.access_restriction_policy_cardview_days_tuesday) as TextView
        this.daysTextViews[3] = itemView.findViewById(
            R.id.access_restriction_policy_cardview_days_wednesday) as TextView
        this.daysTextViews[4] = itemView.findViewById(
            R.id.access_restriction_policy_cardview_days_thursday) as TextView
        this.daysTextViews[5] = itemView.findViewById(
            R.id.access_restriction_policy_cardview_days_friday) as TextView
        this.daysTextViews[6] = itemView.findViewById(
            R.id.access_restriction_policy_cardview_days_saturday) as TextView

        this.wanPolicyStateText = itemView.findViewById(
            R.id.access_restriction_policy_cardview_status) as TextView
        this.menuImageButton = itemView.findViewById(
            R.id.access_restriction_policy_cardview_menu) as ImageButton
        this.removeImageButton = itemView.findViewById(
            R.id.access_restriction_policy_remove_btn) as ImageButton
      }
    }
  }
  ...
  internal class WANAccessPolicyException : DDWRTCompanionException {
    constructor() {}
  ...
  companion object {
    ...
    private val LOG_TAG = AccessRestrictionsWANAccessTile::class.java.simpleName
