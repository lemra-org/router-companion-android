...
public class WirelessIfaceTile extends DDWRTTile<NVRAMInfo>
  ...
  public WirelessIfaceTile(@NonNull String iface, @NonNull Fragment parentFragment,
      @NonNull Bundle arguments, @Nullable Router router) {
    this(iface, null, parentFragment, arguments, router);
  }

  public WirelessIfaceTile(@NonNull String iface, @Nullable String parentIface,
    ...
    ((TextView) this.layout.findViewById(R.id.tile_status_wireless_iface_title)).setText(
        this.iface);
  ...
  @NonNull public static String escapeString(@NonNull final String string) {
  ...
  @NonNull public String getIface() {
    return iface;
  }
  ...
  @Nullable @Override protected Loader<NVRAMInfo> getLoader(int id, Bundle args) {
    return new AsyncTaskLoader<NVRAMInfo>(this.mParentFragmentActivity) {
      ...
      @NonNull private NVRAMInfo getIfaceNvramInfo(@NonNull String wlIface) throws Exception {

        //                if (DDWRTCompanionConstants.TEST_MODE) {
        if (Utils.isDemoRouter(mRouter)) {
          return new NVRAMInfo().setProperty(
        ...
        try {
          nvramInfoTmp =
              SSHUtils.getNVRamInfoFromRouter(mParentFragmentActivity, mRouter, mGlobalPreferences,
                  wlIface + "_radio", wlIface + "_mode", wlIface + "_hwaddr", wlIface + "_ifname",
                  ...
                  wlIface + "_key3", wlIface + "_key4", wlIface + "_wep_bit",
                  wlIface + "_passphrase");
        } finally {
          ...
          if (!isNullOrEmpty(phyIface)) {
            ...
            if (!(isNullOrEmpty(celsius) || isNullOrEmpty(fahrenheit))) {
              nvramInfo.setProperty(wlIface + "_temperature", celsius
                  + DEGREE_SYMBOL
                  + CELSIUS
                  + " ("
                  + fahrenheit
            ...
            final int hasSysClassFolder =
                SSHUtils.runCommands(mParentFragmentActivity, mGlobalPreferences, mRouter,
            ...
            if (rxBps != null) {
              nvramInfo.setProperty(wlIface + "_rx_rate_human_readable",
                  rxBps + " B (" + org.rm3l.router_companion.utils.FileUtils.byteCountToDisplaySize(
                      rxBps) + ")");
            }
            if (txBps != null) {
              nvramInfo.setProperty(wlIface + "_tx_rate_human_readable",
                  txBps + " B (" + org.rm3l.router_companion.utils.FileUtils.byteCountToDisplaySize(
                      txBps) + ")");
            }
            ...
            if (procNetDevNetworkData == null) {
              ...
              if (receive != null) {
                ...
                nvramInfo.setProperty(wlIface + "_rx_packets",
                    String.format("%s (%s)", receive.getRxPackets(), rxErrors <= 0 ? "no error"
                        : (rxErrors + String.format(" error%s", rxErrors > 1 ? "s" : ""))));
              }
              ...
              if (transmit != null) {
                ...
                nvramInfo.setProperty(wlIface + "_tx_packets",
                    String.format("%s (%s)", transmit.getTxPackets(), txErrors <= 0 ? "no error"
                        : (txErrors + String.format(" error%s", txErrors > 1 ? "s" : ""))));
              }
            }
          }
              ...
              SSHUtils.getManualProperty(mParentFragmentActivity, mRouter, mGlobalPreferences,
                  String.format(
                      "%s=$(echo $((`wl -i %s phy_tempsense | awk {' print $1 '}`/2+20))); "
                          + "echo \"C:$%s\"; echo \"F:$(($%s*9/5+32))\"", phyIfaceVarNameInRouter,
                      phyIface, phyIfaceVarNameInRouter, phyIfaceVarNameInRouter));
      ...
      @NonNull private long[] parseFloatDataFromOutput(@Nullable final String[] output) {
      ...
      @Nullable private ProcNetDevNetworkData gatherNetworkUsageFromProcNetDev(
        ...
        try {
          ...
          for (int base = 0; base < gatheredData.size(); base += offset) {
            ...
            procNetDevNetworkDataMap.put(ifaceName,
                new ProcNetDevNetworkData(ifaceName, Long.parseLong(gatheredData.get(base + 1)),
   ...
   */
  @Override public void onLoadFinished(@NonNull Loader<NVRAMInfo> loader,
  ...
  private void updateTileDisplayInfo(@Nullable NVRAMInfo data,
    ...
    if (wlSsid != null) {
      ...
      ((TextView) layout.findViewById(R.id.tile_status_wireless_iface_details_ssid)).setText(
          this.wifiSsid);
    }
    ...
    radioView.setChecked("1".equals(
        data.getProperty(this.iface + "_radio", data.getProperty(this.parentIface + "_radio"))));
    ...
    final String netmode = data.getProperty(this.iface + "_net_mode",
        data.getProperty(this.parentIface + "_net_mode",
            defaultValuesIfNotFound ? EMPTY_VALUE_TO_DISPLAY : null));
    if (netmode != null) {
      final String mode;
    ...
    final String temperatureProperty = data.getProperty(this.iface + "_temperature",
        data.getProperty(this.parentIface + "_temperature",
            defaultValuesIfNotFound ? EMPTY_VALUE_TO_DISPLAY : null));
    ...
    final String channelProperty = data.getProperty(this.iface + "_channel",
        data.getProperty(this.parentIface + "_channel",
            defaultValuesIfNotFound ? EMPTY_VALUE_TO_DISPLAY : null));
    ...
    final String txPwrProperty = data.getProperty(this.iface + "_txpwr",
        data.getProperty(this.parentIface + "_txpwr",
            defaultValuesIfNotFound ? EMPTY_VALUE_TO_DISPLAY : null));
    ...
    final String noiseProp = data.getProperty(this.iface + "_noise",
        data.getProperty(this.parentIface + "_noise",
            defaultValuesIfNotFound ? EMPTY_VALUE_TO_DISPLAY : null));
  ...
  private class WirelessSecuritySettingsActivityResultListener
    ...
    @Override public void onResultCode(int resultCode, Intent data) {
      try {
        ...
        switch (resultCode) {
          case Activity.RESULT_OK:
            ...
            final Bundle token = new Bundle();
            token.putString(DDWRTMainActivity.ROUTER_ACTION,
    ...
    public void onRouterActionSuccess(@NonNull RouterAction routerAction, @NonNull Router router,
      ...
      Utils.displayMessage(mParentFragmentActivity,
          "Security Settings updated for WiFi network '" + wifiSsid + "'", Style.CONFIRM);
      //            Update info right away
