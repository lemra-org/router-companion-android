...
public class WifiSharingQrCodeFragment extends Fragment {
  ...
  @Override public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    final FloatingActionButton shareButton =
        (FloatingActionButton) rootView.findViewById(R.id.tile_status_wireless_iface_qrcode_share);
    ...
    final String mWifiQrCodeString =
        String.format("WIFI:S:%s;T:%s;P:%s;%s;", escapeString(mSsid), wifiEncryptionType,
            escapeString(nullToEmpty(wifiPassword)), mSsid.isEmpty() ? "H:true" : "");

    final ImageView qrCodeImageView =
        (ImageView) rootView.findViewById(R.id.tile_status_wireless_iface_qrcode_image);

    final View loadingView =
        rootView.findViewById(R.id.tile_status_wireless_iface_qrcode_image_loading_view);
          ...
          @Override public void run() {
            try {
              ...
              final Bitmap mBitmap =
                  encodeAsBitmap(mWifiQrCodeString, BarcodeFormat.QR_CODE, outSize.x,
                      outSize.y / 2);
    ...
    shareButton.setOnClickListener(new View.OnClickListener() {
      @Override public void onClick(View view) {
        final View viewToShare =
            rootView.findViewById(R.id.tile_status_wireless_iface_qrcode_view_to_share);
        //Construct Bitmap and share it
        final int width = viewToShare.getWidth();
        final int height = viewToShare.getHeight();
        final Bitmap mBitmapToExport = Bitmap.createBitmap(width > 0 ? width : DEFAULT_BITMAP_WIDTH,
        ...
        if (PermissionChecker.checkSelfPermission(activity,
          ...
          mFileToShare = new File(activity.getCacheDir(), Utils.getEscapedFileName(
              String.format("QR-Code_for_Wireless_Network__%s__on_router_%s", nullToEmpty(mSsid),
                  nullToEmpty(wifiSharingData.mRouterUuid))) + ".png");
          ...
          try {
            ...
            e.printStackTrace();
            Utils.displayMessage(activity, getString(R.string.internal_error_please_try_again),
                Style.ALERT);
            return;
          } finally {
            try {
          ...
          activity.grantUriPermission(activity.getComponentName().getPackageName(), uriForFile,
          ...
          sendIntent.putExtra(Intent.EXTRA_SUBJECT,
              String.format("QR Code for Wireless Network '%s'", mSsid));
          sendIntent.setType("text/html");
          sendIntent.putExtra(Intent.EXTRA_TEXT, fromHtml(String.format("%s%s",
              ((TextView) rootView.findViewById(
                  R.id.tile_status_wireless_iface_qrcode_note)).getText(),
          ...
          if (ActivityCompat.shouldShowRequestPermissionRationale(activity,
            ...
            SnackbarUtils.buildSnackbar(activity,
                "Storage access is required to share WiFi QR Codes.", "OK",
