...
public class UploadAndExecuteScriptRouterAction extends AbstractRouterAction<String[]> {
  ...
  public UploadAndExecuteScriptRouterAction(
      Router router,
  ...
  protected RouterActionResult<String[]> doActionInBackground() {
    ...
    try {
      FirebaseCrashlytics.getInstance()
          .log(
              String.format(
                  "File upload: [%s] => [%s]", mFileAbsolutePath, mRemoteFileAbsolutePath));

      if (!SSHUtils.scpTo(
          mContext, router, globalSharedPreferences, mFileAbsolutePath, mRemoteFileAbsolutePath)) {
      ...
      try {
        SSHUtils.getManualProperty(
            mContext, router, globalSharedPreferences, "rm -rf " + mRemoteFileAbsolutePath);
      } catch (Exception e) {
        e.printStackTrace();
        exception = e;
      }
