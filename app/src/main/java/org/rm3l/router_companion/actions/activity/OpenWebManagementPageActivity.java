  ...
  class WebManagementLoaderTask
    ...
    protected WebManagementLoaderTask.Result doWork() {
      try {
        ...
        final String lanIpAddr =
            nvRamInfoFromRouter.getProperty(NVRAMInfo.Companion.getLAN_IPADDR(), EMPTY_STRING);
        final String lanPort =
            nvRamInfoFromRouter.getProperty(NVRAMInfo.Companion.getHTTP_LANPORT(), EMPTY_STRING);

        final String wanIpAddr =
            nvRamInfoFromRouter.getProperty(NVRAMInfo.Companion.getWAN_IPADDR(), EMPTY_STRING);
        final String wanPort =
            nvRamInfoFromRouter.getProperty(NVRAMInfo.Companion.getHTTP_WANPORT(), EMPTY_STRING);

        if ("1".equals(nvRamInfoFromRouter.getProperty(NVRAMInfo.Companion.getHTTPS_ENABLE()))) {
          lanUrl += "s";
        }
        if ("1"
            .equals(nvRamInfoFromRouter.getProperty(NVRAMInfo.Companion.getREMOTE_MGT_HTTPS()))) {
          wanUrl += "s";
        }
    ...
    protected void thenDoUiRelatedWork(WebManagementLoaderTask.Result result) {
      ...
      if (exception != null) {
        ...
        mErrorTextView.setOnClickListener(
    ...
    return new WebViewClient() {
      ...
      public void onReceivedError(
        ...
        FirebaseCrashlytics.getInstance()
            .log("GOT Page error : code : " + errorCode + ", Desc : " + description);
      ...
      public void onReceivedError(
        ...
        Toast.makeText(
                OpenWebManagementPageActivity.this, "Oh no! " + description, Toast.LENGTH_SHORT)
            .show();
      }
              ...
              public void run() {
                ...
                final View mAuthPromptDialogview =
                    layoutInflater.inflate(
                        R.layout.activity_open_web_mgmt_interface_http_auth_prompt, null);

                final ScrollView contentScrollView =
                    (ScrollView)
                        mAuthPromptDialogview.findViewById(R.id.http_auth_prompt_scrollview);
                ...
                final CheckBox showPasswordPrompt =
                    (CheckBox)
                        mAuthPromptDialogview.findViewById(
      ...
      public void onReceivedSslError(
          WebView view, final SslErrorHandler handler, final SslError error) {
