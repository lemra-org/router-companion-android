...
public class SetNVRAMVariablesAction extends AbstractRouterAction<Void> {
  ...
  @NonNull private final ImmutableList<String> commandsToRunAfterCommit;

  @NonNull private final Context mContext;

  @NonNull private final NVRAMInfo nvramInfo;

  private final boolean withReboot;

  public static RouterActionResult<Void> getRouterActionResult(
    ...
    try {
      ...
      outputFile =
          File.createTempFile(
              SetNVRAMVariablesAction.class.getSimpleName(), ".sh", mContext.getCacheDir());
      ...
      if (!SSHUtils.scpTo(
          mContext, router, globalSharedPreferences, outputFile.getAbsolutePath(), remotePath)) {
        throw new IllegalStateException("Failed to copy set of remote commands to the router");
      }

      final int exitStatus =
          SSHUtils.runCommands(
              mContext, globalSharedPreferences, router, String.format("/bin/sh %s", remotePath));
      if (exitStatus != 0) {
  ...
  public SetNVRAMVariablesAction(
      Router router,
      @NonNull Context context,
      @NonNull NVRAMInfo nvramInfo,
      boolean withReboot,
