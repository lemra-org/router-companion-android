  ...
  @NonNull @Override protected RouterActionResult<String> doActionInBackground() {
    ...
    try {
      ...
      final ImmutableTable.Builder<Long, Integer, Multimap<Integer, Long>> traffDataTableBuilder =
          ImmutableTable.<Long, Integer, Multimap<Integer, Long>>builder().orderRowsBy(
              new Comparator<Long>() {
                @Override public int compare(Long o, Long t1) {
                  if (o == t1) {
                    return 0;
                  }
                  if (o == null) {
      ...
      @SuppressWarnings("ConstantConditions") final Set<Map.Entry<Object, Object>> entries =
          nvramInfo.getData().entrySet();
      ...
      for (final Map.Entry<Object, Object> entry : entries) {
        ...
        final String monthAndYear =
            key.toString().replace("traff-", RouterCompanionAppConstants.EMPTY_STRING);
        ...
        for (final String dailyInOutTraffData : dailyTraffDataList) {
          ...
          dataMap.putAll(dayNum++,
              Lists.newArrayList(Long.parseLong(inTraff) * MB, Long.parseLong(outTraff) * MB));
        }
      ...
      String escapedFileName = Utils.getEscapedFileName("traffdata_"
      ...
      switch (mBackupFileType) {
        case BackupFileType_CSV:
          ...
          final ImmutableSet<Table.Cell<Long, Integer, Multimap<Integer, Long>>> cells =
              traffDataTable.cellSet();
          for (final Table.Cell<Long, Integer, Multimap<Integer, Long>> cell : cells) {
            ...
            for (final Map.Entry<Integer, Collection<Long>> entry : dayInAndOutMap.entrySet()) {
              ...
              Files.append(
                  String.format("%d,%d,%d,%d,%s,%d,%s\n", year, month, day, traffDataForDay[0],
                      FileUtils.byteCountToDisplaySize(traffDataForDay[0]), traffDataForDay[1],
                      FileUtils.byteCountToDisplaySize(traffDataForDay[1])), mLocalBackupFilePath,
                  CHARSET);
            }
          }
          break;
        ...
        default:
          ...
          Files.append(Joiner.on("\n")
              ...
              .useForNull("\n")
              .join(nvramInfoWithTraffData.getData()), mLocalBackupFilePath, CHARSET);
          break;
      }
    } catch (Exception e) {
      e.printStackTrace();
