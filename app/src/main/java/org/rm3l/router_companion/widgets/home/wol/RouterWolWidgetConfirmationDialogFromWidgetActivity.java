...
public class RouterWolWidgetConfirmationDialogFromWidgetActivity extends ConfirmDialogAsActivity {
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    if (intent.getBooleanExtra(LOAD_HOSTS_FROM_PREFS, false)) {
      wolHosts = getSharedPreferences(routerUuid, Context.MODE_PRIVATE).getString(
          WOLWidgetService.WOL_HOST_PREF_KEY, null);
    } else {
    ...
    try {
      mCurrentDevicesList = WakeOnLanTile.GSON_BUILDER.create()
          .fromJson(wolHosts, new TypeToken<ArrayList<Device>>() {
          }.getType());
    ...
    final Set<String> bCastAddrs =
        getSharedPreferences(routerUuid, Context.MODE_PRIVATE).getStringSet(
            WOLWidgetService.WOL_BCAST_ADDRESSES_PREF_KEY, new HashSet<String>());
      ...
      @Override public void onClick(View view) {
        ...
        for (final Device device : mCurrentDevicesList) {
          wolActions[i++] = new WakeOnLANRouterAction(mRouter,
              RouterWolWidgetConfirmationDialogFromWidgetActivity.this, new RouterActionListener() {
            @Override public void onRouterActionSuccess(@NonNull final RouterAction routerAction,
            ...
            @Override public void onRouterActionFailure(@NonNull final RouterAction routerAction,
                  ...
                  @Override public void run() {
                    Toast.makeText(RouterWolWidgetConfirmationDialogFromWidgetActivity.this,
                        String.format("Action '%s' executed but %d error(s) occurred: %s",
                            ...
                            Utils.handleException(exception).first), Toast.LENGTH_SHORT).show();
                  }
                });
              }
            }
          }, getSharedPreferences(RouterCompanionAppConstants.DEFAULT_SHARED_PREFERENCES_KEY,
              Context.MODE_PRIVATE), device, device.getWolPort(), mBroadcastAddresses);
        }
