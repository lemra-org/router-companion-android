...
public class WOLWidgetProvider extends AppWidgetProvider {
  ...
  public void onReceive(final Context context, final Intent intent) {
    ...
    if (appWidgetId != AppWidgetManager.INVALID_APPWIDGET_ID
      ...
      if (routerUuid != null) {
        final Router router =
            RouterManagementActivity.Companion.getDao(context).getRouter(routerUuid);
        if (router == null) {
          Toast.makeText(
                  context, "Invalid Router - entry may have been dropped!", Toast.LENGTH_SHORT)
              .show();
        } else {
          switch (intentAction) {
            case ACTION_REFRESH:
                ...
                try {
                  final ArrayList<Device> devices =
                      WakeOnLanTile.GSON_BUILDER
                          .create()
                          .fromJson(
                              deviceToWakeStr, new TypeToken<ArrayList<Device>>() {}.getType());
                  if (!(devices == null || devices.isEmpty())) {
                    final Device device = devices.get(0);
                    if (device != null) {
                      final Intent wakeIntent =
                          new Intent(
                              context, RouterWolWidgetConfirmationDialogFromWidgetActivity.class);
  ...
  static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int appWidgetId) {
    ...
    if (router == null) {
      ...
      if (BuildConfig.DONATIONS) {
        ...
        wakeAllIntent.putExtra(
            ConfirmDialogAsActivity.MESSAGE, "Are you sure you wish to attempt waking all hosts?");
      }
