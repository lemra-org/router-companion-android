...
class WOLRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory,
    ...
    public RemoteViews getViewAt(int position) {
        ...
        final RemoteViews rv = new RemoteViews(mContext.getPackageName(), R.layout.widget_wol_host_item);
        ...
        if (device == null) {
            ...
            rv.setTextViewText(R.id.widget_wol_host_item_device_details_nic_manufacturer, EMPTY_VALUE_TO_DISPLAY);
        } else {
    ...
    public void onLoadComplete(@Nullable Loader<RouterData<ArrayList<Device>>> loader, @Nullable RouterData<ArrayList<Device>> data) {
        ...
        try {
            mRouterPreferences.edit()
                    .putStringSet(WOLWidgetService.WOL_BCAST_ADDRESSES_PREF_KEY, new HashSet<>(mBcastAddresses))
